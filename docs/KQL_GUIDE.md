...
KQL is a SQL-like declarative query language for detecting security vulnerabilities and code patterns in source code. It allows security researchers to write custom detection rules without modifying the SAST engine's Rust code.
...
Every KQL query has three clauses:
...
KQL supports Rust regex syntax in `MATCHES` operator:
...
KQL provides a powerful, declarative way to detect security vulnerabilities:
