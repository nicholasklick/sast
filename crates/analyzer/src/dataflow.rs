    ...
    T: Clone + Eq + Hash + Debug,
{
    ...
    pub fn analyze(&self, cfg: &ControlFlowGraph) -> DataFlowResult<T> {
        ...
        while let Some(node) = worklist.pop_front() {
            let changed = match self.direction {
                DataFlowDirection::Forward => self.analyze_forward(cfg, &mut result, node),
                DataFlowDirection::Backward => self.analyze_backward(cfg, &mut result, node),
            };
    ...
    fn analyze_forward(
        ...
        let new_out = self.transfer_fn.transfer(node, &merged);
    ...
    fn analyze_backward(
        ...
        // Apply transfer function
        let new_in = self.transfer_fn.transfer(node, &merged);
...
mod tests {
    ...
    impl TransferFunction<String> for DummyTransfer {
        fn transfer(&self, _node: CfgGraphIndex, input: &HashSet<String>) -> HashSet<String> {
            input.clone()
        }
